...
#    define MULTIPLE_WORKER_PROCESS
...
#    ifdef MULTIPLE_WORKER_PROCESS
static std::vector<int> g_pids;
#    endif // MULTIPLE_WORKER_PROCESS
...
static int _set_no_block_socket(int fd, void* on_error_context, _on_error_callback_t on_error)
{
    ...
    if (flag < 0)
    {
        if (nullptr != on_error)
        {
            on_error(on_error_context, ez::base::format("Unable to get socket flag, because: %s(%d).", ERRMSG, ERRCODE));
        }
        return flag;
    }
    ...
    if (flag < 0)
    {
        if (nullptr != on_error)
        {
            on_error(on_error_context, ez::base::format("Unable to set socket flag with `O_NONBLOCK`, because: %s(%d).", ERRMSG, ERRCODE));
        }
    }
    return rv;
}
...
#    ifdef MULTIPLE_WORKER_PROCESS
static inline void _handle_term_signal(int sig)
{
    for (const auto& pid : g_pids)
    {
        ::kill(pid, SIGTERM);
    }
}
...
void ez::net::epoller::accept_all(int fd)
{
    int rv = 0;
    while (true)
    {
        // READ event on server fd, need to accept a new client:
        std::shared_ptr<struct sockaddr> addr((struct sockaddr*)::malloc(sizeof(struct sockaddr())), ::free);
        ...
        CONSOLE("process %d accepted fd: %d.", ::getpid(), fd);
...
void ez::net::epoller::start_worker_loop()
{
    int rv = 0;
    while (true)
    {
...
#    ifdef MULTIPLE_WORKER_PROCESS
    for (size_t i = 0; i < 4; i++)
    {
        ...
        {
            // ERROR!
            ON_ERROR("Unable to create child process, because: %s(%d).", ERRMSG, ERRCODE);
        }
    }
    // Parent process wait.
    while (1)
